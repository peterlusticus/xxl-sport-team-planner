...
$(document).ready(function () {
    ...
    $("body").on("click", ".selectable-row", function () {
        ...
        scheduler.createDataProcessor(function (entity, action, data, id) {
            switch (action) {
                case "create":
                    console.log(coursidx)
                    firebase.database().ref('courses/' + coursidx + '/events').limitToLast(1).once('value').then(function(snapshot) {
                        let num = 0
                        if (snapshot.exists())  {
                            num=parseInt(Object.keys(snapshot.val())[0])+1
                        }
                        firebase.database().ref('courses/' + coursidx + '/events/' + num).set({
                            text: data.text,
